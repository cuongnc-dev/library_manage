class User < ApplicationRecord
  ...
  scope :list_users_follow_book, -> book_id, id do
    joins(:follow_books).select("users.*").
      where "follow_books.book_id = ? and users.id <> ?", "#{book_id}", "#{id}"
  end
  scope :list_user_duo_borrow_book, -> today do
    joins(:requests).where "requests.end_day = ?", "#{today}"
  end
